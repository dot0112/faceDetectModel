...
def preprocess_image(image_path: str) -> tf.Tensor:
    ...
    # image_tensor = shape(24, 24, 3)
    image_tensor = tf.io.read_file(image_path)
    image_tensor = tf.image.decode_jpeg(image_tensor, channels=3)
    image_tensor = tf.image.resize(image_tensor, [24, 24])
...
def preprocess_class_label(class_label: int) -> tf.Tensor:
    ...
    class_label = tf.reshape(class_label, [1])
...
def preprocess_bbox_label(bbox_label: list[float]) -> tf.Tensor:
    ...
    bbox_label = tf.reshape(bbox_label, [4])
...
def preprocess_data(
    ...
    try:
        ...
        return [
            tf.zeros((24, 24, 3), dtype=tf.float32),
...
def preprocess_wrapper(
    ...
    image_tensor.set_shape((24, 24, 3))
    class_label.set_shape((1))
    bbox_label.set_shape((4))
